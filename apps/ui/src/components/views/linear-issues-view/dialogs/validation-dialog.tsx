...
interface ValidationDialogProps {
  ...
  /**
   * Absolute path of the project whose codebase this result was scanned against.
   * Linear issues aren't tied to a repo the way GitHub issues are, so this is shown
   * to let the user catch a mismatch (open project isn't what the issue is about).
   */
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          {scannedProjectPath && (
            <p className="text-xs text-muted-foreground">
              Scanned against: {scannedProjectPath.split('/').pop()}
            </p>
          )}
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          {validationResult?.verdict === 'valid' && onCreateFeature && (
