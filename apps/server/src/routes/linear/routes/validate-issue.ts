/**
 * POST /validate-issue endpoint - Validate a Linear issue using provider abstraction (async)
 *
...
import { writeValidation, isValidIssueIdentifier } from '../../../lib/linear-validation-storage.js';
...
import type { LinearComment } from './list-comments.js';
...
async function runValidation(
  ...
  // Emit start event
  const startEvent: LinearIssueValidationEvent = {
  ...
  try {
    // Build the prompt (include comments if provided)
    ...
    const prompts = await getPromptCustomization(settingsService, '[ValidateLinearIssue]');
...
${basePrompt}`;
    ...
    const result = await streamingQuery({
      ...
      onText: (text) => {
        responseText += text;
        // Emit progress event
        const progressEvent: LinearIssueValidationEvent = {
    ...
    logger.info(`Issue ${issueIdentifier} validation complete: ${validationResult.verdict}`);

    // Store the result
    await writeValidation(projectPath, issueIdentifier, {
      issueIdentifier,
      issueTitle,
      projectPath,
    ...
    const completeEvent: LinearIssueValidationEvent = {
    ...
    logError(error, `Issue ${issueIdentifier} validation failed`);

    // Emit error event
    const errorEvent: LinearIssueValidationEvent = {
...
export function createValidateIssueHandler(
  ...
  return async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Transform LinearComment[] to ValidationComment[] if provided
      ...
      logger.info(
        `[ValidateLinearIssue] Received validation request for issue ${issueIdentifier}` +
      ...
      if (!isValidIssueIdentifier(issueIdentifier)) {
        res.status(400).json({
          success: false,
          error: 'issueIdentifier is required and must look like "ENG-123"',
        });
        return;
      }
      ...
      logger.info(`Starting async validation for issue ${issueIdentifier}: ${issueTitle}`);

      // Create abort controller and atomically try to claim validation slot
      // This prevents TOCTOU race conditions
      const abortController = new AbortController();
      if (!trySetValidationRunning(projectPath, issueIdentifier, abortController)) {
        res.json({
          success: false,
          error: `Validation is already running for issue ${issueIdentifier}`,
        });
        return;
      }
      ...
      runValidation(
        ...
        .finally(() => {
          clearValidationStatus(projectPath, issueIdentifier);
        });

      // Return immediately
      res.json({
        success: true,
        message: `Validation started for issue ${issueIdentifier}`,
        issueIdentifier,
      });
    } catch (error) {
      logError(error, `Linear issue validation failed`);
